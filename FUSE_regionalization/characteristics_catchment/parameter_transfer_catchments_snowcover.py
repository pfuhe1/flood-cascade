...
class dummy_result:
	def __init__(self,catchid,val):
		self.val = val
		self.catchid = catchid
	def get(self):
		return self.catchid,self.val
...
tile = '/home/pu17449/data2/modis_snowcover/modis_snowcover_mean_2001-2014.tif'
...
mean_out = '/home/pu17449/data2/parameter_transfer_catchments/catchment_vals/'+var+'.pkl'
...
def processtile(ftile,catchment,catchid,tmpfile):
	...
	if not os.path.exists(tmpfile):
		cmd = ['gdalwarp','-of','GTiff','-dstnodata','255','-co','COMPRESS=DEFLATE','-cutline',catchment,'-r','near','-crop_to_cutline',ftile,tmpfile]
		...
		if ret!=0:
			...
			#return None
			# Hack to set values for catchments too small to contain full grid cells
			# (10 catchments that give an error)
			if int(catchid) == 4243200:
				return catchid,38.7
			else:
				return catchid,0.0
	...
	#missingval = 3.4028234663852886e+38
